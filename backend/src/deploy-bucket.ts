...
// the ONE thing LiveOpsPoppy creates OUTSIDE its stack — so (a) it's tagged with the
...
//
// Bucket ops take an injected S3 client so the deploy/teardown logic is unit-testable
 ...
 *  (liveopspoppy-deploy-*), lowercased and DNS-safe. */
export function deployBucketName(accountId: string, region: string): string {
  return `liveopspoppy-deploy-${accountId}-${region}`.toLowerCase();
}
