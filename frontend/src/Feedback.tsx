...
// and a donate flow of our own: a user who installs three poppies must find the same four
// things in the same place. The element is vendored verbatim from the AgentsPoppy SDK
// (`npm run sync-feedback`; `npm run check-feedback` fails if the copy drifts).
...
const POPPY_ID = "com.liveopspoppy.desktop";

/** Where a bug goes — the public issue tracker, mirroring `bugsUrl` in extension.json. */
const BUGS_URL = "https://github.com/leonct74/liveops-poppy/issues";

// The tab calls the AgentsPoppy feedback API itself; the only thing it needs from the host
// is `openExternal` (a sandboxed frame can't open an OS window), which our bridge has.
...
export function Feedback() {
  ...
  useEffect(() => {
    ...
    el.setAttribute("name", "LiveOpsPoppy");
    ...
    <div className="stack">
      <div>
        ...
        <p className="muted" style={{ margin: 0 }}>
          Tell us how LiveOpsPoppy is doing. Your rating shows on the AgentsPoppy catalogue, and
