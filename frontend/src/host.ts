// Minimal host-bridge client (inlined — the AgentsPoppy SDK isn't published to npm, so
// like MailPoppy/TrafficPoppy we mirror the tiny wire contract instead of taking a
// cross-repo build dependency). Protocol mirrors @agentspoppy/extension-sdk bridge.ts:
...
/** True when we're inside the AgentsPoppy host frame. Outside it (a plain `vite dev` tab,
 *  or a screenshot run) NOBODY will ever answer, so the call must fail immediately rather
 *  than leave the UI on a skeleton until the two-minute timeout expires. */
export const hasHostBridge = (): boolean => window.parent !== window;

function call<T>(method: string, params: unknown[], timeoutMs = 120_000): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    if (!hasHostBridge()) {
      reject(new Error("LiveOpsPoppy isn't running inside AgentsPoppy, so it can't reach your AWS account."));
      return;
    }
    seq += 1;
...
export const host = {
  ...
  /**
   * Commerce (extension-sdk §4, capability `commerce:purchase`).
   *
   * ⚠ `isPurchased` is the gate and the host verifies ownership SERVER-SIDE. Never cache
   * the result as a durable "unlocked" flag or let a client-only boolean stand in for it.
   */
...
/** The per-studio subscription priced in the developer dashboard (DESIGN.md §10). */
export const STUDIO_PRODUCT = "studio";
