// LiveOpsPoppy collector core — every pure decision the collector makes, with no AWS
// imports so all of it unit-tests in milliseconds. store.ts applies these decisions to
// DynamoDB.
//
// The schema contract (key literals + limits) lives in shared/src/keys.ts so the admin
// backend and this Lambda can never drift; it is re-exported here because core.test.ts
// locks the literals and the rest of the Lambda imports them from one place.
...
export function eventSkFor(
  name: string,
  isNewName: boolean,
  distinctNamesToday: number,
  cardCap: number = DEFAULT_CARD_CAP,
): string {
